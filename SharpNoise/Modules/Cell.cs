...
namespace SharpNoise.Modules
{
    ...
    public class Cell : Module
    {
        ...
        /// <summary>
        /// Gets or sets the seed value used by the Cell generator
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets or sets the type of cells that will be generated
        /// </summary>
        public CellType Type { get; set; }

        public delegate double DistanceFunction(double x, double y, double z);

        public int MinkowskyCoefficient { get; set; }

        /// <summary>
        /// Gets or sets a custom function that will be used to determine the cell
        /// a point belongs to
        /// </summary>
        public DistanceFunction CustomDistanceFunction { get; set; }
                    ...
                    for (var xCur = xint - 2; xCur <= xint + 2; xCur++)
                    {
                        ...
                        switch (Type)
                        {
                            ...
                            case CellType.Chebychev:
                                dist = Math.Max(Math.Abs(xDist), Math.Max(Math.Abs(yDist), Math.Abs(zDist)));
                                break;
                            ...
                            case CellType.Minkowsky:
                                double ddx = Math.Pow(Math.Abs(xDist), MinkowskyCoefficient);
                                double ddy = Math.Pow(Math.Abs(yDist), MinkowskyCoefficient);
                                double ddz = Math.Pow(Math.Abs(zDist), MinkowskyCoefficient);

                                dist = Math.Pow(ddx + ddy + ddz, 1D / MinkowskyCoefficient);
                                break;

                            case CellType.Quadratic:
                                dist = xDist * xDist + yDist * yDist + zDist * zDist + xDist * yDist + xDist * zDist + yDist * zDist;
                                break;

                            case CellType.Custom:
                                dist = CustomDistanceFunction(xDist, yDist, zDist);
                                break;
                        }
            ...
            : base(0)
        {
            ...
            Seed = DefaultSeed;
            Type = CellType.Voronoi;
            MinkowskyCoefficient = 4;
        }
    }
}
